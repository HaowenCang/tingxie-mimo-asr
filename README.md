...
- 媒体库支持批量导出所选转写，也可按文件夹递归导出并选择是否保留目录层级；重名文件自动避让，不覆盖已有内容。
- 媒体库支持多级文件夹、拖放整理、文件夹重命名/移动/安全删除，以及录音显示名称与关联转写标题同步修改。
- 媒体库支持选择多个未转写媒体后批量加入任务；已转写、部分完成和正在队列中的媒体会自动跳过，失败媒体可选择是否重试。
- 批量任务默认同时准备 3 个音频的探测、提取和切片；可改为任意正整数或无限制。媒体之间只开放一个 API 转写通道，严格按入队顺序推进，单个媒体内部继续使用现有 RPM 与自适应切片并发。
- 开启“转写后自动生成智能速览”后，新转写保存即进入串行后台分析队列；应用启动时也会补排尚未生成速览的历史记录，无需先打开详情页。
- 底部“智能速览队列”可查看正在生成、排队、重试、失败，以及历史缺失或过期的速览；支持打开原文、手动生成/重试和按当前转写版本移出。
- 未完成队列会保存轻量恢复清单；应用异常退出后任务显示为“上次任务已中断”，由用户决定是否重试，不会自动消耗 API 额度。
...
- [批量转写与并行准备设计](./docs/BATCH_TRANSCRIPTION_PIPELINE.md)
- [后台智能速览与本地准备并行](./docs/BACKGROUND_ANALYSIS_AND_PREPARATION.md)
