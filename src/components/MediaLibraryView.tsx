...
import { CheckSquare, ChevronDown, ChevronRight, ExternalLink, FileAudio, FileClock, FilePlus2, FileText, Folder, FolderInput, FolderPlus, Library, Move, Pencil, Play, Search, Square, Trash2, Upload, X } from 'lucide-react'
      ...
      <aside className="folder-rail" onDragOver={(event) => event.preventDefault()} onDrop={(event) => handleFolderDrop(event)}>
        ...
        <div className="folder-tree">{folderTree.map((node) => {
          ...
          return <div key={node.folder.id} className={`folder-tree-item${active ? ' active' : ''}`} data-folder-depth={node.depth} style={{ '--folder-depth': node.depth } as CSSProperties}>
            <GlassContextMenu ariaLabel={`${node.folder.name} 文件夹操作`} entries={folderContextEntries(node.folder)}><div className="folder-tree-row" draggable onDragStart={(event) => event.dataTransfer.setData('application/x-tingxie-folder-id', node.folder.id)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.stopPropagation(); handleFolderDrop(event, node.folder.id) }}>
        ...
        <div className="library-table" role="table" aria-label="媒体文件">
          ...
          <div ref={tableScrollRef} className="library-table-scroll" data-virtualized="true">{rows.length ? <div className="library-virtual-rows" style={{ height: `${rowVirtualizer.getTotalSize()}px`, position: 'relative' }}>{rowVirtualizer.getVirtualItems().map((virtualRow) => {
            ...
            return <div key={rowKey} className="library-virtual-row" style={{ position: 'absolute', top: 0, left: 0, width: '100%', transform: `translateY(${virtualRow.start}px)` }}>
              {row.kind === 'asset' ? <GlassContextMenu ariaLabel={`${row.asset.displayName} 操作`} entries={rowContextEntries(row, rowKey, linkedTranscript)} onOpenChange={(open) => { if (open) { if (!selected.has(rowKey)) setSelected(new Set([rowKey])); setFocusedRowKey(rowKey) } }}><div className={`library-table-row${selected.has(rowKey) ? ' selected' : ''}${focusedRowKey === rowKey ? ' focused' : ''}`} role="row" tabIndex={0} draggable onDragStart={(event) => { const keys = selected.has(rowKey) ? selected : new Set([rowKey]); const assetIds = selectionIds(keys, 'asset'); const transcriptIds = selectionIds(keys, 'transcript'); if (assetIds.length) event.dataTransfer.setData('application/x-tingxie-media-ids', JSON.stringify(assetIds)); if (transcriptIds.length) event.dataTransfer.setData('application/x-tingxie-transcript-ids', JSON.stringify(transcriptIds)) }} onClick={openRow} onKeyDown={(event) => { if (event.key === 'Enter') openRow() }}>
              ...
              </div></GlassContextMenu> : <GlassContextMenu ariaLabel={`${row.transcript.fileName} 操作`} entries={rowContextEntries(row, rowKey)} onOpenChange={(open) => { if (open) { if (!selected.has(rowKey)) setSelected(new Set([rowKey])); setFocusedRowKey(rowKey) } }}><div className={`library-table-row legacy-transcript-row${selected.has(rowKey) ? ' selected' : ''}${focusedRowKey === rowKey ? ' focused' : ''}`} role="row" tabIndex={0} draggable onDragStart={(event) => { const keys = selected.has(rowKey) ? selected : new Set([rowKey]); const transcriptIds = selectionIds(keys, 'transcript'); if (transcriptIds.length) event.dataTransfer.setData('application/x-tingxie-transcript-ids', JSON.stringify(transcriptIds)) }} onClick={openRow} onKeyDown={(event) => { if (event.key === 'Enter') openRow() }}>
