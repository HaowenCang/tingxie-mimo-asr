    ...
    <m.section className="settings-modal" variants={dialogPanel} initial="initial" animate="animate" exit="exit" onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="settings-title">
      ...
      <footer><button className="secondary-button" disabled={(!selectedHasKey && !apiKey) || busy !== null} onClick={test}>{busy === 'test' && <LoaderCircle className="spin" size={16} />}测试连接</button><button className="primary-button compact" disabled={busy !== null || !hasChanges} onClick={save}>{busy === 'save' && <LoaderCircle className="spin" size={16} />}保存设置</button></footer></> : section === 'ai' ? <AIProviderSettings settings={aiSettings} onChange={onAISettingsChange} onSave={onSaveAIProvider} onDelete={onDeleteAIProvider} onSelect={onSelectAIProvider} onTest={onTestAIProvider} /> : <>
        <div className="settings-content personalization-settings">
          ...
          <section className="preference-group"><div className="preference-group-heading"><strong>批量转写管线</strong><span>本地准备可以并行；媒体之间的 API 转写始终按队列顺序执行</span></div><div className="preferences-grid">
            <label><span>本地切片准备方式</span><GlassSelect ariaLabel="本地切片准备方式" className="settings-select" value={preferences.preparationMode} options={[{ value: 'sequential', label: '顺序准备' }, { value: 'fixed', label: '自定义并行' }, { value: 'unlimited', label: '无限制' }]} onValueChange={(value) => {
              if (value === 'unlimited' && preferences.preparationMode !== 'unlimited' && !window.confirm('无限制模式会同时启动当前批次的全部本地音频准备任务，可能显著占用 CPU、内存和临时磁盘空间。确认继续吗？')) return
              setPreferences({ ...preferences, preparationMode: value as AppPreferences['preparationMode'] })
            }} /></label>
            {preferences.preparationMode === 'fixed' && <label><span>同时准备的音频数</span><input className="settings-number-input" aria-label="同时准备的音频数" type="number" min="1" step="1" value={preferences.preparationConcurrency} onChange={(event) => {
              const value = Number(event.target.value)
              if (Number.isSafeInteger(value) && value > 0) setPreferences({ ...preferences, preparationConcurrency: value })
            }} /></label>}
          </div>{preferences.preparationMode === 'unlimited' && <p className="settings-warning">无限制模式不设置应用级并行上限；API 转写仍严格按媒体队列顺序执行。磁盘空间不足或系统无法创建进程时，对应任务会安全失败而不会改变其他转写结果。</p>}</section>
